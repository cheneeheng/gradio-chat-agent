...
counter_component = ComponentDeclaration(
    ...
    permissions=ComponentPermissions(readable=True, writable_via_actions_only=True),
    tags=["demo"]
)
...
def set_handler(inputs: dict[str, Any], snapshot: StateSnapshot):
    ...
    diff = [StateDiffEntry(path=f"{COUNTER_ID}.value", op="replace", value=val)]
...
def increment_handler(inputs: dict[str, Any], snapshot: StateSnapshot):
    ...
    diff = [StateDiffEntry(path=f"{COUNTER_ID}.value", op="replace", value=new_val)]
...
def reset_handler(inputs: dict[str, Any], snapshot: StateSnapshot):
    ...
    diff = [StateDiffEntry(path=f"{COUNTER_ID}.value", op="replace", value=0)]
...
set_action = ActionDeclaration(
    ...
    preconditions=[],
    effects={"may_change": [f"{COUNTER_ID}.value"]},
...
reset_action = ActionDeclaration(
    ...
    preconditions=[],
    effects={"may_change": [f"{COUNTER_ID}.value"]},
    permission=ActionPermission(confirmation_required=True, risk=ActionRisk.MEDIUM, visibility=ActionVisibility.USER)
)
