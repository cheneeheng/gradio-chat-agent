...
memory_component = ComponentDeclaration(
    ...
    permissions=ComponentPermissions(
        readable=True,
        writable_via_actions_only=True
    ),
    tags=["system", "memory"]
)
...
def remember_handler(
    ...
    # Get current components (shallow copy of dict, deep copy of values done by engine usually,
    # but here we should be careful to copy the specific component we modify)
...
remember_action = ActionDeclaration(
    ...
    preconditions=[],
    effects={
        "may_change": [f"{MEMORY_COMPONENT_ID}.*"]
    },
