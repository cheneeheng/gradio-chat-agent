...
def run_example():
    ...
    print(f"New Value: {latest_snapshot.components['demo.counter']['value']}")
    ...
    assert latest_snapshot is not None
    print(
        f"Actual Value (unchanged): {latest_snapshot.components['demo.counter']['value']}"
    )
    ...
    for i, step_res in enumerate(plan_sim_res["data"]):
        print(
            f"Step {i + 1} ({step_res['action_id']}) Simulated: {step_res['simulated']}"
        )
    ...
    assert latest_snapshot is not None
    print(
        f"Actual Value (still 10): {latest_snapshot.components['demo.counter']['value']}"
    )
