...
class TestBearerTokens:
    ...
    def test_token_validation_expiry(self, setup_in_memory):
        api, repo = setup_in_memory
        # Expired
        repo.create_api_token("u1", "E", "expired-token", expires_at=datetime.now() - timedelta(seconds=1))
        assert repo.validate_api_token("expired-token") is None
        
        # Future
        repo.create_api_token("u1", "F", "valid-token", expires_at=datetime.now() + timedelta(hours=1))
    ...
    def test_token_lifecycle_sql(self):
        ...
        # Expiry in SQL
        repo.create_api_token("u1", "E", "tok-exp", expires_at=datetime.utcnow() - timedelta(seconds=1))
    ...
    def test_api_permission_denied(self, setup_in_memory):
        ...
        assert res2["code"] == 0 # Wait, I added `user_id != owner_user_id` check
